...
select by prefix keep group and any var that starts with gd
...
rerrange by selection if pair with everything fucntion, keeps litter
number and everything else relocate does same as everything function
...
what if I only select one variable…? select keeps it in its respective
column
...
use ‘filter()’ to filter out rows filter gets rid of everything false
...
when want to look for more than five pups born alive can also do greater
...
“command shift m” gives pipe operator \|\> means AND THEN… assumes that
whatever comes out of pipe then goes into the next automatically;
already knows which dataframe to use
...
learning assessment: import and tidy the pups data in a piped chain of
commands
